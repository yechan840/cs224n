...
class NMT(nn.Module):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(dim=2)).squeeze(dim=2) # (b, src_len, h), (b, h, 1) -> (b, src_len, 1)  -> (b, src_len)
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(dim=1), enc_hiddens).squeeze(dim=1) # (b, 1, src_len, b, src_len, 2h) -> (b, 1, 2h) -> (b, 2h)
